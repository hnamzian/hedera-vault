...
func (h *AccountHandler) handleCreate(ctx context.Context, req *logical.Request, data *framework.FieldData) (*logical.Response, error) {
	...
	operator_account_buf, err := storage.NewStorage(req).WithContext(ctx).WithKey(req.ClientToken, "", id).Read()
	if err != nil {
	...
	operator_key_buf, err := storage.NewStorage(req).WithContext(ctx).WithKey(req.ClientToken, "", operator_account.KeyID).Read()
	if err != nil {
		return nil, errwrap.Wrapf("read account failed: {{err}}", err)
	}
