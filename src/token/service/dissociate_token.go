...
func (t_svc *TokenService) DissociateWithToken(tokenID string, account, operatorID string) (*hedera.Status, error) {
	...
	user_key, _ := t_svc.k_svc.GetKey(user_account.KeyID)
	if err != nil {
	...
	return ht.DissociateWithToken(tokenID, user_account.AccountID, user_key.PrivateKey)
}
